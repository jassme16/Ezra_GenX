...
class payrollMainInformation(models.Model):
        ...
        if len(payroll_details) > 0:
            for payroll_detail in payroll_details:
                ...
                if payroll_dict['basic_pay_amount'] > 0:
                    if payroll_detail.is_reliever == False or payroll_detail.is_additional_employee == True:
    ...
    def generateExcelFile2(self):
        ...
        if len(model_payroll_detail) > 0:
            ...
            #Title
            sheet.write(2,5,self.payroll_attendance.company_assign.name)
            sheet.write(3,5,str(self.payroll_attendance.schedule_datefrom) + ' - ' + str(self.payroll_attendance.schedule_dateto))
            #sheet.write(4,4,self.payroll_releasedate)
            ...
            for employee in model_payroll_detail:
                if len(employee.employee_id.bank_account_id) > 0:
                    ...
                    sheet.write(int_rowindex,53,employee.other_deductions ,styleColumns)
                    sheet.write(int_rowindex,54,employee.deductions,styleColumns)
                    sheet.write(int_rowindex,55,employee.net_pay,styleColumns)
                    int_rowindex +=1
            ...
            for employee in model_payroll_detail:
                ...
                sheet.write(int_rowindex,53,curr_other_deductions,styleColumns1)
                sheet.write(int_rowindex,54,curr_deduction,styleColumns1)
                sheet.write(int_rowindex,55,curr_net,styleColumns1)
            ...
            sheet.write(int_rowindex,53,round(sum(employee.other_deductions for employee in model_payroll_detail),2),styleColumns1)
            sheet.write(int_rowindex,54,round(sum(employee.deductions for employee in model_payroll_detail),2),styleColumns1)
            sheet.write(int_rowindex,55,round(sum(employee.net_pay for employee in model_payroll_detail),2),styleColumns1)



            int_rowindex +=2

            sheet.write(int_rowindex,44,round(total_gross_check,2),styleColumns1)
            int_rowindex +=1
            sheet.write(int_rowindex,44,round(total_gross_debit,2),styleColumns1)

            int_rowindex +=2
            sheet.write(int_rowindex,44,round(total_gross_check + total_gross_debit,2),styleColumns1)
